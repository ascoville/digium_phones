	...
	foreach ($phonebook['entries'] as $entryid=>$entry) {
		...
		if ($customexten == false) {
			...
			if ($user != null && $user['voicemail'] != null && $user['voicemail'] != "novm") {
				...
				$output[] = '    <actions>';
				$output[] = '      <action id="dial" dial="' . htmlspecialchars($entry['extension']) . '" dial_prefix="" label="Dial" name="Dial" />';
				$output[] = '      <action id="pickupcall" dial="'. htmlspecialchars($entry['extension'])  .'" dial_prefix="**" label="Pickup" name="Pickup"/>';
				$output[] = '      <action id="sendtovm" dial="'. htmlspecialchars($entry['extension'])  .'" dial_prefix="*" label="Send to VM" name="Send to VM"/>';
				$output[] = '      <action id="blindxfer" dial="'. htmlspecialchars($entry['extension'])  .'" dial_prefix="" label="BlindXfer" name="Blind Transfer"/>';
				$output[] = '    </actions>';
